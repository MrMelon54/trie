...
func sp(a string) *string { return &a }
...
func TestTrie_Put(t *testing.T) {
	...
	var expected = &Trie[string]{Prefix: []byte{0xF0, 0x9F, 0x91}, Value: sp("short"), Children: &[256]*Trie[string]{
		0x10: {Prefix: []byte{0x10}, Value: sp("modified")},
		0xA8: {Prefix: []byte{0xA8}, Value: sp(`👨`), Children: &[256]*Trie[string]{
			0xE2: {Prefix: []byte{0xE2, 0x80, 0x8D}, Value: sp(`👨‍`), Children: &[256]*Trie[string]{
				0xF0: {Prefix: []byte{0xF0, 0x9F, 0x94, 0xA7}, Value: sp(`👨‍🔧`)},
			}},
		}},
	}}
...
var (
	tr = T{Prefix: []byte{0xF0, 0x9F, 0x91}, Children: &[256]*T{
		0xA8: {Prefix: []byte{0xA8}, Value: sp("👨") /*F0 9F 91 A8*/, Children: &[256]*T{
			0xE2: {Prefix: []byte{0xE2, 0x80, 0x8D}, Children: &[256]*T{
				0xE2: {Prefix: []byte{0xE2}, Children: &[256]*T{
					0x9A: {Prefix: []byte{0x9A}, Children: &[256]*T{
						0x95: {Prefix: []byte{0x95, 0xEF, 0xB8, 0x8F}, Value: sp("👨‍⚕️") /*F0 9F 91 A8 E2 80 8D E2 9A 95 EF B8 8F*/},
						0x96: {Prefix: []byte{0x96, 0xEF, 0xB8, 0x8F}, Value: sp("👨‍⚖️") /*F0 9F 91 A8 E2 80 8D E2 9A 96 EF B8 8F*/},
					}},
					0x9C: {Prefix: []byte{0x9C, 0x88, 0xEF, 0xB8, 0x8F}, Value: sp("👨‍✈️") /*F0 9F 91 A8 E2 80 8D E2 9C 88 EF B8 8F*/},
					0x9D: {Prefix: []byte{0x9D, 0xA4, 0xEF, 0xB8, 0x8F, 0xE2, 0x80, 0x8D, 0xF0, 0x9F}, Children: &[256]*T{
						0x91: {Prefix: []byte{0x91, 0xA8}, Value: sp("👨‍❤️‍👨") /*F0 9F 91 A8 E2 80 8D E2 9D A4 EF B8 8F E2 80 8D F0 9F 91 A8*/},
						0x92: {Prefix: []byte{0x92, 0x8B, 0xE2, 0x80, 0x8D, 0xF0, 0x9F, 0x91, 0xA8}, Value: sp("👨‍❤️‍💋‍👨") /*F0 9F 91 A8 E2 80 8D E2 9D A4 EF B8 8F E2 80 8D F0 9F 92 8B E2 80 8D F0 9F 91 A8*/},
					}},
				}},
			}},
		}},
...
func TestTrie_GetAllKeys(t *testing.T) {
	...
	var inputs = map[string][]string{
		"/api/user/list": {"", "/api/user", "/api/user/list"},
		"/api/test":      {""},
		"/api/user/li":   {"", "/api/user"},
	}

	for key, expected := range inputs {
		res := tr.GetAllKeys([]byte(key))
...
func TestTrie_GetByString(t *testing.T) {
	...
	var inputs = map[string]result{
		"":                {sp("root"), true},
		"/api/user/list":  {sp("users list"), true},
